...
class SolverRegistry {
 public:
  typedef Solver<Dtype>* (*Creator)(const SolverParameter&);//定义函数
  typedef std::map<string, Creator> CreatorRegistry;

  static CreatorRegistry& Registry() {
    static CreatorRegistry* g_registry_ = new CreatorRegistry();//因为g_registry_是static,所以只会new一次
    return *g_registry_;
  }
  ...
  static Solver<Dtype>* CreateSolver(const SolverParameter& param) {
    ...
    return registry[type](param);//实例化
  }
